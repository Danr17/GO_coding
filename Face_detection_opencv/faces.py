...
def detectFaces(img_path, display=False):
    try:
        if img_path.lower().endswith('.png'):
            ...
            color_img = imageio.mimread(img_path)[0] # frist frame of the gif
            color_img = cv2.cvtColor(color_img, cv2.COLOR_BGR2RGB)
    except Exception as e:
        raise e
    
    # scale images to fit with 1280x720
    ...
    if display:
        ...
        for (x, y, w, h) in faces:
            cv2.rectangle(color_img, (x, y), (x+w, y+w), (0, 255, 0), 3)
...
def main():
    # open file dialog to chose imput image
    root = tk.Tk()
    root.withdraw()
    ...
    x = input('Save results to {}?  [y/n]: '. format(os.path.basename(json_path)))
